			 ...
			 unsigned int ioctl, unsigned long arg)
{
	...
	case KVM_GET_PRIVATE_GFN_RANGE: {
		struct kvm_priv_mem_info pminfo;
		pminfo.start = vcpu->priv_mem_start;
                pminfo.size = vcpu->priv_mem_end - vcpu->priv_mem_start;
	...
	case KVM_SET_PRIVATE_GFN_RANGE: {
		struct kvm_priv_mem_info pminfo;
		...
		if ((pminfo.start + pminfo.size) < pminfo.start)
			goto out;
		vcpu->priv_mem_start = pminfo.start;
		vcpu->priv_mem_end = pminfo.start + pminfo.size;
		r = 0;
		break;
	}
	case KVM_HAS_DEVICE_ATTR:
